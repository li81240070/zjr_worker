 ...
 * 抢险抢修订单详情页面
 */
public class EmerDetailsActivity extends BaseActivity implements View.OnClickListener {
    private static final int UN_OVEW_REQUEST_CALL_PHONE = 100;
    private static final int OVEW_REQUEST_CALL_PHONE = 101;
    public static EmerDetailsActivity finishActivity; // 注销销毁的activity
//    private boolean ISCOMMIT = false;
    ...
    private BacklogOrder.DataMapEntity.OfferEmergencysEntity entity; // 订单实体类
...
//    private UserInfo userInfo;                  // 匠人信息
    ...
    protected void initData() {
        ...
        // 获取订单实体
        entity = (BacklogOrder.DataMapEntity.OfferEmergencysEntity) getIntent().getSerializableExtra("order");
//        if (entity == null) {
...
//        }
        ...
        if (orderEnd == null && orderPush != null) {
            orderEnd = new LatLng(orderPush.getDataMap().getX(), orderPush.getDataMap().getY());
        }
...
//            orderPrice_Tv.setText(String.valueOf(orderPush.getDataMap().getTotal_price()));      // 订单价格
...
//                }
//                WindowManager manager = EmerDetailsActivity.this.getWindowManager();
//                int wid = manager.getDefaultDisplay().getWidth();
//                int hei = manager.getDefaultDisplay().getHeight();
//                final OrderPhotoDialog dialog = new OrderPhotoDialog(EmerDetailsActivity.this, imgurl, wid, hei);
//                dialog.show();
...
//                    return;
//                }
                Intent toMap = new Intent(EmerDetailsActivity.this, MapLocationActivity.class);
            ...
            case R.id.toolbar_right_Tv: // 上传照片
                if (entity != null && entity.getMobile() != null) {
                    if (!"4".equals(entity.getOrder_status())) {
                        Toast.makeText(EmerDetailsActivity.this, "无法上传照片", Toast.LENGTH_SHORT).show();
                        return;
                    }
                } else {
                    if (orderPush.getDataMap().getOrder_status() != 4) {
                ...
                Intent commit = new Intent(EmerDetailsActivity.this, CommitPictureActivity.class);
...
//        dialog.getWindow().setLayout((int) (width * 0.85), (int) (height * 0.4));
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 2: // 完工状态
                    ...
                    Intent intent = new Intent(EmerDetailsActivity.this, HomeActivity.class);
    ...
    Runnable changePriceTask = new Runnable() {
        ...
        public void run() {
            ...
            try {
                status = OkHttpClientManager.getAsyn(url, EmerDetailsActivity.this);
                if (status.code() != 401) {
                    message.what = 1; // 修改价格
                } else {
