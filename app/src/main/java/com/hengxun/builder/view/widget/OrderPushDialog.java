 ...
 */
public class OrderPushDialog extends Dialog implements View.OnClickListener, OnGetRoutePlanResultListener, OnGetGeoCoderResultListener {
    ...
    private int isKnock; // 是否是抢单状态 1:抢单 2：派单
    private OrderPush bean;//订单对象
    private BacklogOrder.DataMapEntity.OfferOrdersEntity entity;
...
//    private LocationClient mLocationClient;
    ...
    public void setHelpOrderClickListener(HelpOrderClickListener helpOrderClickListener) {
        this.helpOrderClickListener = helpOrderClickListener;
    }
     ...
     **/
    public OrderPushDialog(Context context, int wid, int hei, int isKnock, OrderPush bean,
    ...
    public OrderPushDialog(Context context, int wid, int hei, int isKnock, BacklogOrder.DataMapEntity.OfferOrdersEntity bean,
                           double mLatitude, double mLongtitude) {
        super(context, R.style.OrderInforDialog);
        this.context = context;
        this.wid = wid;
...
//        initLocation();
    ...
    private void initData() {
        ...
        if (bean != null && bean.getDataMap() != null && bean.getDataMap().getOrder_status() == 1) { // 正常抢单
            String x, y;
            x = String.valueOf(bean.getDataMap().getOrder_x());
            y = String.valueOf(bean.getDataMap().getOrder_y());
            ...
            if (bean != null) {
                ...
                if (bean.getDataMap().getTotal_price() >= 0){
                    double d = bean.getDataMap().getTotal_price();
                ...
                orderDetails_TV.setText(bean.getDataMap().getCustomer_comment());
            }

        } else if (isKnock == 1) { // 派单
            String x, y;
            x = String.valueOf(bean.getDataMap().getOrder_x());
            y = String.valueOf(bean.getDataMap().getOrder_y());
            LatLng orderEnd = new LatLng(Float.parseFloat(x), Float.parseFloat(y)); // 订单结束位置
...
//            order_bottom_btn_textTV.setText("抢单");
            if (entity != null) {
                subject_orderTV.setText(entity.getService());//项目名
                if (Double.parseDouble(entity.getPrice()) >= 0)
                    subject_priceTV.setText("￥ " + Double.parseDouble(entity.getPrice()));//价格
                time_orderTV.setText(DateUtil.getStrTime(String.valueOf(entity.getAppoint_time())) + "");//时间
                address_orderTV.setText(entity.getAddress());//地址
                end = entity.getAddress();
                orderDetails_TV.setText(entity.getComment());
            }
...
//    }
    ...
    private void drivePlan() {
        ...
        LatLng end = null;
        if (bean != null) {
            end = new LatLng(bean.getDataMap().getOrder_x(),
                    bean.getDataMap().getOrder_y());
        } else if (entity != null) {
            end = new LatLng(Double.parseDouble(entity.getX()),
                    Double.parseDouble(entity.getY()));
        }
        //起点与终点
