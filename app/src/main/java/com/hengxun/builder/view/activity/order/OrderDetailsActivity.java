...
public class OrderDetailsActivity extends BaseActivity implements View.OnClickListener {
    ...
    public static OrderDetailsActivity finishActivity; // 注销销毁的activity
...
//    private UserInfo userInfo;                  // 匠人信息
    ...
    protected void initData() {
        ...
        if (entity != null) {
            ...
            if (orderPush.getDataMap().getOrder_status() != 4) {
                toolbar_right_Tv.setVisibility(View.GONE);
            }
            if (orderPush.getDataMap().getAdjustable() != 1 || orderPush.getDataMap().getOrder_status() == 4) {
                orderChange_btn.setTextColor(getResources().getColor(R.color.forget_tv_lines));
                orderChange_btn.setBackgroundResource(R.drawable.shape_order_unchange_btn);
            }
        }
        ...
        if (orderEnd == null && orderPush != null) {
            orderEnd = new LatLng(orderPush.getDataMap().getOrder_x(), orderPush.getDataMap().getOrder_y());
        }
//        userInfo = (UserInfo) getIntent().getSerializableExtra("userInfo");
        ...
        if (entity != null && entity.getImages() != null) {
            ...
            orderPrice_Tv.setText(StringUtils.changeToMoney(Double.valueOf(entity.getPrice())));      // 订单价格
            ...
            for (int i = 0; i < orderPush.getDataMap().getImages().size(); i++) {
                String imgurl = AppApi.ORDER_IMG + orderPush.getDataMap().getImages().get(i) + "_200.jpg";
                img.add(imgurl);
            }
            orderAddPhoto_Rv.setAdapter(adapter); // 添加图片数据
            adapter.addData(img);

            orderMessages_Tv.setText(orderPush.getDataMap().getCustomer_comment()); // 订单描述

            orderPrice_Tv.setText(StringUtils.changeToMoney(orderPush.getDataMap().getTotal_price()));      // 订单价格
            orderType_Tv.setText(orderPush.getDataMap().getService().getGroup_name());
            orderAddress_Tv.setText(orderPush.getDataMap().getAddress());
            appointment_Tv.setText(DateUtil.getStringTime(String.valueOf(orderPush.getDataMap().getAppoint_time())));
        }
        ...
        adapter.setOnItemClickListener(new OrderDetailsAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (entity != null && entity.getImages() != null) {
                    ...
                    imgurl = AppApi.ORDER_IMG + orderPush.getDataMap().getImages().get(position) + "_640.jpg";
                }
                WindowManager manager = OrderDetailsActivity.this.getWindowManager();
                int wid = manager.getDefaultDisplay().getWidth();
                int hei = manager.getDefaultDisplay().getHeight();
                final OrderPhotoDialog dialog = new OrderPhotoDialog(OrderDetailsActivity.this, imgurl, wid, hei);
                dialog.show();
        ...
        if (entity != null && entity.getMobile() != null) {
            ...
            if (!(orderPush.getDataMap().getImages() != null) ||
                    !(orderPush.getDataMap().getImages().size() > 0)) { // 如果图片为空则不显示recyclerview
                orderAddPhoto_Rv.setVisibility(View.GONE);
                orderPhotos_Iv.setVisibility(View.VISIBLE);
            }
        }
        ...
        if (entity != null) {
            /* nothing to do */
        } else if (orderPush != null && orderPush.getDataMap().getOrder_status() == 6) {
            orderDetails_Btn.setVisibility(View.VISIBLE);
            orderDetails_Btn.setText("付款");
            orderDetails_Btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent i = new Intent(OrderDetailsActivity.this, PayActivity.class);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.callMerchants_Rl:
                ...
                } else if (orderPush.getDataMap().getOrder_status() == 5
                        || orderPush.getDataMap().getOrder_status() == 6) {
                    Toast.makeText(this, "不可以给用户拨打电话", Toast.LENGTH_SHORT).show();
                } else {
                    if (orderPush.getDataMap().getOrder_status() != 5 &&
                            orderPush.getDataMap().getOrder_status() != 6 &&
                            orderPush.getDataMap().getMobile() != null && !orderPush.getDataMap().getMobile().equals("")) {

                        Toast.makeText(this, "将给" + orderPush.getDataMap().getCustomerName() + "拨打电话，手机号" +
                                orderPush.getDataMap().getMobile(), Toast.LENGTH_SHORT).show();
                        ...
                        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE)
                            ...
                            callPhone(orderPush.getDataMap().getMobile());
                        }
                    }
                }
                break;
            ...
            case R.id.orderLocation_Tv:
                if (entity != null && entity.getModel() == 2) {
                    Toast.makeText(this, "该订单不可定位", Toast.LENGTH_SHORT).show();
                        ...
                        || orderPush.getDataMap().getOrder_status() == 5)) {
                    Toast.makeText(this, "该订单不可定位", Toast.LENGTH_SHORT).show();
                } else {
                    Intent toMap = new Intent(OrderDetailsActivity.this, MapLocationActivity.class);
                    Bundle bundle = new Bundle();
...
//                        orderChange_btn.setBackgroundResource(R.drawable.shape_order_unchange_btn);
                    ...
                    if (orderPush.getDataMap().getAdjustable() == 1) {
                        if (orderPush.getDataMap().getOrder_status() == 3 ||
                                orderPush.getDataMap().getOrder_status() == 2) {
                            showDialog(); // 更改价格
                        }
                    } else {
                        Toast.makeText(this, "该订单不可改价", Toast.LENGTH_SHORT).show();
...
//                        orderChange_btn.setBackgroundResource(R.drawable.shape_order_unchange_btn);
            ...
            case R.id.sure_Tv: // 确定更改
                ...
                if (entity != null) {
                    ...
                    if (Float.parseFloat(price) <= Float.parseFloat(String.valueOf(orderPush.getDataMap().getTotal_price()))) {
                        Toast.makeText(this, "修改价格不能低于原始价格", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }

                if (Double.parseDouble(price) >= 1000000.0000000) {
                    Toast.makeText(this, "价钱输入格式不正确", Toast.LENGTH_SHORT).show();
                    return;
                }
            ...
            case R.id.toolbar_right_Tv: // 上传照片
                if (entity != null && entity.getMobile() != null) {
                    if (!"4".equals(entity.getOrder_status())) {
                        Toast.makeText(OrderDetailsActivity.this, "无法上传照片", Toast.LENGTH_SHORT).show();
                        return;
                    }
                } else {
                    if (orderPush.getDataMap().getOrder_status() != 4) {
                ...
                Intent commit = new Intent(OrderDetailsActivity.this, CommitPictureActivity.class);
                Bundle data = new Bundle();
                if (entity != null) {
                    data.putSerializable("order_id", entity.getOrder_id());
                } else {
                    data.putSerializable("order_id", String.valueOf(orderPush.getDataMap().getOrder_id()));
                }
...
//    @Override
//    public void finish() {
//        if (orderPush != null && orderPush.getDataMap().getOrder_status() == 6) {
//            Toast.makeText(OrderDetailsActivity.this, "请立即完成付款", Toast.LENGTH_SHORT).show();
//        } else {
//            super.finish();
//        }
//    }
...
//        dialog.getWindow().setLayout((int) (width * 0.7), (int) (height * 0.4));
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:  // 修改价格
                    ...
                    dialog.dismiss();
                    Toast.makeText(OrderDetailsActivity.this, "价格修改成功", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2: // 完工状态
                    ...
                    break;
                case 3:  // 修改出错
                    Toast.makeText(OrderDetailsActivity.this, "价格修改失败", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                    break;
            }
    ...
    Runnable changePriceTask = new Runnable() {
        ...
        public void run() {
            ...
            try {
                status = OkHttpClientManager.getAsyn(url, OrderDetailsActivity.this);
                ...
                if (status.code() != 401) {
                    ...
                    if (info.getCode() == 200) {
                        message.what = 1; // 修改价格失败
                    } else if (info.getCode() == 500) {
                        message.what = 3; // 修改价格
                    } else if (info.getCode() == 401) {
                        errToken();
                    }
                }
            } catch (Exception e) {
