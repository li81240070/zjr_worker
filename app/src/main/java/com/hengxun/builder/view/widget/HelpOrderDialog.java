 ...
 */
public class HelpOrderDialog extends Dialog implements View.OnClickListener, OnGetRoutePlanResultListener, OnGetGeoCoderResultListener {
    ...
    private TextView order_to_mapTV, title_ordorTV, order_bottom_btn_textTV, subject_orderTV,
            subject_priceTV, time_orderTV, address_orderTV, orderDetails_TV;
    ...
    private BacklogOrder.DataMapEntity.OfferEmergencysEntity emerOrder;
...
//    private LocationClient mLocationClient;
     ...
     **/
    public HelpOrderDialog(Context context, int wid, int hei, int isKnock, HelpOrder bean,
    ...
    public HelpOrderDialog(Context context, int wid, int hei, int isKnock, BacklogOrder.DataMapEntity.OfferEmergencysEntity bean,
                           double mLatitude, double mLongtitude) {
        super(context, R.style.OrderInforDialog);
        this.context = context;
        this.wid = wid;
    ...
    private void initData() {
        ...
        if (isKnock == 1) {
            ...
            if (helpOrder != null) {
                ...
                orderDetails_TV.setText(helpOrder.getDataMap().getDetail().getComment());
            }
        ...
        if (isKnock == 2) {
            ...
            if (emerOrder != null) {
                ...
                x = String.valueOf(emerOrder.getX());
                y = String.valueOf(emerOrder.getY());
            }
            LatLng orderEnd = new LatLng(Float.parseFloat(x), Float.parseFloat(y)); // 订单结束位置
            double dis = getDistance(orderStart, orderEnd); // 距离
            if (dis > 1000) { // 单位 米
            ...
            if (emerOrder != null) {
                subject_orderTV.setText(emerOrder.getService());//项目名
                if (Double.parseDouble(emerOrder.getPrice()) >= 0) {
                    double d = Double.parseDouble(emerOrder.getPrice());
                    String st = String.format("%.2f", d);
                    subject_priceTV.setText("¥ " + st);//价格
                }
                time_orderTV.setText(DateUtil.getStrTime(String.valueOf(emerOrder.getAppoint_time())) + "");//时间
                address_orderTV.setText(emerOrder.getAddress());//地址
                end = emerOrder.getAddress();
                orderDetails_TV.setText(emerOrder.getComment());
//                if (end == null || end.equals("")) {
//                    Toast.makeText(context, "定位失败, 请打电话联系用户", Toast.LENGTH_SHORT).show();
//                }
            }
...
//    }
        ...
        switch (v.getId()) {
            ...
            case R.id.order_bottomBtn://底部圆钮
                if (onOrderClickListener != null) {
                    onOrderClickListener.onOrderclick(order_bottomBtn, isKnock, helpOrder);
                }
                break;
        }
    }
    ...
    private void drivePlan() {
        ...
        //起点与终点
        PlanNode stNode = PlanNode.withCityNameAndPlaceName(city != "" ? city : "大连市", start);
...
//        LatLng end = null;
//        if (helpOrder != null) {
//            end = new LatLng(Double.parseDouble(helpOrder.getDataMap().getDetail().getX()),
//                    Double.parseDouble(helpOrder.getDataMap().getDetail().getY()));
//        } else if (emerOrder != null) {
//            end = new LatLng(Double.parseDouble(emerOrder.getX()),
//                    Double.parseDouble(emerOrder.getY()));
//        }
//        //起点与终点
